...
Drops uns entirely (it contains large per-perturbation DE tables that account
for most of the file size). Only obs, var, layers, and X are retained.

Usage:
    python scripts/prepare_norman_subset.py \
        --input  /path/to/norman_perturbation.h5ad \
        --output tests/fixtures/norman_subset.h5ad \
        [--n-obs 50] [--n-vars 50] [--seed 42]
"""
import argparse
import numpy as np
...
def main():
    ...
    p.add_argument("--output", default="tests/fixtures/norman_subset.h5ad")
    p.add_argument("--n-obs",  type=int, default=50)
    p.add_argument("--n-vars", type=int, default=50)
    ...
    sub.uns.clear()  # drop DE tables and other large uns entries

    print(f"  subset shape: {sub.shape}")
    print(f"  obs columns:  {list(sub.obs.columns)}")
    print(f"  layers:       {list(sub.layers.keys())}")

    sub.write_h5ad(args.output, compression="gzip", compression_opts=6)

    import os
    kb = os.path.getsize(args.output) / 1024
    print(f"written {kb:.1f} KB → {args.output}")
